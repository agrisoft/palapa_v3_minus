...
$.get(_api + "listmetalayer", function (data) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    console.log(extent)
    $('#list-type').append('<div class="col-sm-6 col-md-3 p0 wow fadeInUp animated" data-wow-delay="0.2s"><div class="box-two proerty-item"><div class="item-thumb"><a><img src="' + image2 + '"</a></div><div class="item-entry overflow"><div id="ltitle"><a href="property-1.html">' + listdata[i]['title'] + ' </a></div><div class="dot-hr"></div><span class="pull-left"><b>' + listdata[i]['keywords'] + '</b></span><span class="proerty-price pull-right"><i id="' + listdata[i]['identifier'] + '" class="material-icons preview" title="Lihat peta" class="cursor_pointer">location_on<div style="display:none"><div id="ident">' + listdata[i]['identifier'] + '</div><div id="minx">' + minx + '</div><div id="miny">' + miny + '</div><div id="maxx">' + maxx + '</div><div id="maxy">' + maxy + '</div></div></i><i class="material-icons" title="Lihat metadata" data-toggle="modal" data-target="#metaData" class="cursor_pointer">info</i><i class="material-icons" title="Download" data-toggle="modal" data-target="#downloadModal" class="cursor_pointer">cloud_download</i></span><div class="property-icon"><b>' + array[0][0] + '</b></div></div></div>');
  }
});
...
$("#searchbtn").on('click', function() {
   window.searchextent = searchmap.getView().calculateExtent(searchmap.getSize());
